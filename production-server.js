...
// Email HTML generators (same as in server.ts)
...
<head>
  ...
  <style>
    ...
    .details-box { background-color: #f8fafc; border-left: 4px solid #667eea; padding: 20px; margin: 20px 0; border-radius: 4px; }
    .detail-row { display: flex; margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px solid #e2e8f0; }
    .detail-row:last-child { border-bottom: none; margin-bottom: 0; padding-bottom: 0; }
    .detail-label { font-weight: 600; color: #475569; width: 140px; flex-shrink: 0; }
    .detail-value { color: #1e293b; flex: 1; }
    .highlight { background-color: #dbeafe; border: 2px solid #3b82f6; padding: 15px; border-radius: 6px; margin: 20px 0; text-align: center; }
    .highlight strong { color: #1e40af; font-size: 18px; }
    .pet-item { display: inline-block; background-color: #e0e7ff; color: #4338ca; padding: 4px 12px; border-radius: 12px; margin-right: 8px; margin-bottom: 8px; font-size: 14px; font-weight: 500; }
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <div>Great news! Your grooming appointment has been confirmed. We're excited to pamper your furry friend${petNames.length > 1 ? 's' : ''}!</div>
      <div class="highlight"><strong>📅 ${date} at ${data.booking.scheduled_time_start} - ${data.booking.scheduled_time_end}</strong></div>
      <div class="details-box">
        <div class="detail-row"><span class="detail-label">Service:</span><span class="detail-value">${data.booking.service_name || 'Grooming Service'}</span></div>
        <div class="detail-row"><span class="detail-label">Pet${petNames.length > 1 ? 's' : ''}:</span><span class="detail-value">${petNames.map((name) => `<span class="pet-item">${name}</span>`).join('')}</span></div>
        <div class="detail-row"><span class="detail-label">Groomer:</span><span class="detail-value">${data.groomer.first_name} ${data.groomer.last_name}</span></div>
        <div class="detail-row"><span class="detail-label">Location:</span><span class="detail-value">${data.booking.address}, ${data.booking.city}, ${data.booking.state}</span></div>
        <div class="detail-row"><span class="detail-label">Total Amount:</span><span class="detail-value"><strong>$${data.booking.total_amount.toFixed(2)}</strong></span></div>
      </div>
      <p style="color: #475569; margin-top: 25px;"><strong>What to expect:</strong><br>• Your groomer will arrive at the scheduled time<br>• The grooming service will be done in our mobile van<br>• We'll send you before and after photos<br>• Payment will be processed after service completion</p>
    </div>
    <div class="footer">
...
function generateGroomerEmailHTML(data) {
  const petNames = data.pets.map(p => p.name);
  const date = new Date(data.booking.scheduled_date).toLocaleDateString('en-US', {
    weekday: 'long',
    year: 'numeric',
...
<body>
  <div class="container">
    <div class="header"><h1>🐾 New Booking Assignment</h1></div>
    <div class="content">
...
// API endpoint for booking approval emails
...
// Fallback to index.html for Angular routing (except /api routes)
app.use((req, res) => {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Email API available at: /api`);
